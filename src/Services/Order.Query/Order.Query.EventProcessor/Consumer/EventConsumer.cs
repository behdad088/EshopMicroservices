    ...
    where TView : class, IProjection<TView, TEvent>
{
    ...
    public async Task Consume(ConsumeContext<CloudEvent<TEvent>> context)
    {
        ...
        EnrichLoggerScope(_logger, context, @event);
    ...
    private static void EnrichLoggerScope(
        ILogger logger,
        ConsumeContext<CloudEvent<TEvent>> context,
        TEvent @event)
    {
        logger.ForContext(LogProperties.EventId, context.Message.Id)
            .ForContext(LogProperties.EventType, @event.EventType)
            .ForContext(LogProperties.EventSource, context.Message.Source)
            .ForContext(LogProperties.StreamId, @event.StreamId);
    }
